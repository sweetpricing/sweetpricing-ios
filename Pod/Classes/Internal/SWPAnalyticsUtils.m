    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:supportPath
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:supportPath
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error]) {
            SWPLog(@"error: %@", error.localizedDescription);
        }
    }
...
static id SWPCoerceJSONObject(id obj)
{
    ...
    if ([obj isKindOfClass:[NSArray class]]) {
        ...
        for (id i in obj)
            [array addObject:SWPCoerceJSONObject(i)];
        return array;
    }
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        for (NSString *key in obj) {
            if (![key isKindOfClass:[NSString class]])
                SWPLog(@"warning: dictionary keys should be strings. got: %@. coercing "
                       @"to: %@",
                       [key class], [key description]);
            dict[key.description] = SWPCoerceJSONObject(obj[key]);
        }
        return dict;
    }
    ...
    SWPLog(@"warning: dictionary values should be valid json types. got: %@. "
           @"coercing to: %@",
...
NSDictionary *SWPCoerceDictionary(NSDictionary *dict)
{
    ...
    return SWPCoerceJSONObject(dict);
}
